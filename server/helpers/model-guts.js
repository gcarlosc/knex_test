...
module.exports = ({
  ...
  const findAll = (schema, filters) => knex.withSchema(schema).select(selectableProps)
    .from(tableName)
    .timeout(timeout)

  const find = (schema, filters) => knex.withSchema(schema).select(selectableProps)
    .from(tableName)
    .where(filters)
    .timeout(timeout)
  ...
  const findById = (schema, id) => knex.withSchema(schema).select(selectableProps)
    .from(tableName)
    .where({ id })
    .timeout(timeout)
